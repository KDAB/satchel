...
**satchel-demo/**
  ...
  Shows how to use `#[should_panic]` and expected panic messages in tests.
...
**Test Harness:**
  The test harness in `satchel` exposes a getter for all registered tests in the current crate. All example crates now use the shared test runner from `examples/test-runner`, which provides a unified API for running tests and benchmarks, including support for `#[should_panic]` and expected panic messages.
...
4. **No need to implement `run_tests` manually:**
  The shared test runner in `examples/test-runner` provides all necessary logic, including support for `#[should_panic]` and expected panic messages.
